...
void Scene::addChild(Object* obj) {
    switch (obj->getClassType()) {
        case EMesh: {
            ...
            if (mesh->isEmitter()) {
                m_emitter.push_back(mesh->getEmitter());
            }
            break;
        }
        case EEmitter: {
...
void Scene::sampleEmitterDirection(const Intersection& its,
    ...
    } else if (m_emitter.size() == 1) {
        m_emitter[0]->sampleDirection(sample, its, ds, spectrum);
    } else {
        size_t emitter_num = m_emitter.size();
        float emitter_pdf = 1.f / emitter_num;

        // Randomly pick a emitter
        size_t index =
            std::min((size_t)(sample.x() * emitter_num), emitter_num - 1);
        Emitter* emitter = m_emitter[index];
...
float Scene::pdfEmitterDirection(const DirectionSample& ds) const {
    if (m_emitter.size() == 1) {
        return m_emitter[0]->pdfDirection(ds);
    } else {
        const Emitter* emitter = reinterpret_cast<const Emitter*>(ds.obj);
        return emitter->pdfDirection(ds) / m_emitter.size();
    }
}
