...
static __device__ __inline__ float
intersectPlane(const float3& origin, const float3& direction,
               const float3& normal, const float3& point) {
    // Skipping checks for non-zero denominator since we know that ray
    // intersects this plane
    return -(dot(normal, origin - point)) /
           dot(normal, direction);
}

// Offset the hit point using integer arithmetic
static __device__ __inline__ float3
offset(const float3& hit_point, const float3& normal) {
...
static __device__ __inline__ void refine_and_offset_hitpoint(
    const float3& original_hit_point, const float3& direction,
    const float3& normal, const float3& p,
    float3& back_hit_point, float3& front_hit_point) {
    using namespace optix;
